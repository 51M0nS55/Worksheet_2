...
def showcase(points, function, title, line_color):
	plt.figure()
	plt.title(title)
	plt.scatter(points[:, 0], points[:, 1], color='blue')
	hull = function(points)
	hull = np.vstack((hull, hull[0]))
	plt.plot(hull[:, 0], hull[:, 1], color=line_color)
	plt.xlabel('$x$')
	plt.ylabel('$y$')
	plt.show()
...
if __name__ == '__main__':
	...
	showcase(points, graham_scan, 'Graham scan', 'red')
	showcase(points, jarvis_march, 'Jarvis march', 'green')
	showcase(points, quickhull, 'Quickhull', 'black')
	showcase(points, monotone_chain, 'Monotone chain', 'purple')